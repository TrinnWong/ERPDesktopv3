...
as
	...
	select @movimientoDetalleId + ROW_NUMBER() OVER(ORDER BY pb.ProductoBaseId ASC), 
	@movimientoid, 
	pb.ProductoBaseId,
	ROW_NUMBER() OVER(ORDER BY pb.ProductoBaseId ASC),
	sum(pb.Cantidad)*VD.Cantidad,			0,			0,	
	--Si tiene productos base, insertar cantidad 0 ya que no se debe de inventariar, solo dejar registro		
	Cantidad = sum(pb.Cantidad)*VD.Cantidad,
	v.UsuarioCreacionId,GETDATE()
	from doc_ventas V
	inner join doc_ventas_detalle vd on vd.VentaId = V.VentaId
	...
	where v.VentaId = @pVentaId AND
	V.Activo = 1 
	group by pb.ProductoBaseId,v.UsuarioCreacionId,VD.Cantidad
