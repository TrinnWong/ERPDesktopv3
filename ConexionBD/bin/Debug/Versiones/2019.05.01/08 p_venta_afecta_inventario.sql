...
as
	...
	Cantidad = case when count(distinct pb.ProductoBaseId) > 0 then 0 else vd.Cantidad end,			VD.PrecioUnitario,			VD.Total,	
	--Si tiene productos base, insertar cantidad 0 ya que no se debe de inventariar, solo dejar registro		
	Disponible = case when count(distinct pb.ProductoBaseId) > 0 then 0 else vd.Cantidad end,
	...
	V.Activo = 1
	group by vd.ProductoId,vd.Cantidad,VD.PrecioUnitario,VD.Total,v.UsuarioCreacionId
	...
	where v.VentaId = @pVentaId AND
	V.Activo = 1
	group by pb.ProductoBaseId,pb.Cantidad,v.UsuarioCreacionId
