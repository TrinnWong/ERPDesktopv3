if exists (
	select 1
	from sysobjects
	where name = 'p_doc_ventas_rec_sinc'
)
drop proc p_doc_ventas_rec_sinc
go



create proc p_doc_ventas_rec_sinc
@pSucursalId int,
...
--ERPJalisco.dbo.doc_web_carrito: FK_Carrito_Ventas
	...
	BEGIN TRY  
		...
		begin
			...
			/***Crear relación antes de sincronizar****/
	...
	BEGIN CATCH  
		--ROLLBACK TRAN
		set @pError = 'Ocurrió un error al sincronizar tablas'
	END CATCH;  