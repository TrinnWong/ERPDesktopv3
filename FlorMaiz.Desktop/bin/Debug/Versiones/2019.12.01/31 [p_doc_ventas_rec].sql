ALTER proc [dbo].[p_doc_ventas_rec]
@pSucursalId int,
...
as
	...
	begin
		...
		BEGIN TRY	
			...
			if(isnull(@porc_rec,0) = 0)
			begin
				set @pError = 'Ocurrió un error, revise la configuración relacionada al corte de caja'
				return
			end
			...
			from doc_ventas v
			where ventaId = (
				select case when  min(VentaId) -1 <= 0 then 0
							else min(VentaId) -1
			...
			--exec ERPtEMP..p_corte_caja_generacion @pCajaId ,
			--@pUsuarioId ,@pFechaHoraCorte,0 ,@pPermitirCorteEnCero 
