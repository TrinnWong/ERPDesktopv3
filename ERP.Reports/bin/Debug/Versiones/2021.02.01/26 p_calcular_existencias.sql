...
as
	...
	begin
		...
		set @costoPromedio = case when isnull(@existencia,0) = 0then 0 else 
								cast((@ultimoValorCtoProm + @actualValorMov) / @existencia as decimal(15,2))
							end
	...
	update doc_inv_movimiento_detalle
		...
		update doc_inv_movimiento_detalle
		set 		
			ValCostoPromedio =case when isnull(ValCostoPromedio,0) <0 then 0 else isnull(ValCostoPromedio,0) end,
			ValCostoUltimaCompra = case when isnull(ValCostoUltimaCompra,0) <0 then 0 else isnull(ValCostoUltimaCompra,0) end,
