...
as
	...
	begin
		...
		select top 1 @ultimoValorCtoProm = isnull(pe.CostoPromedio,0) * @existenciaAnt
		from doc_inv_movimiento m
		inner join doc_inv_movimiento_detalle md on md.MovimientoId = m.MovimientoId
		...
		set @costoPromedio = (@ultimoValorCtoProm + @actualValorMov) / @existencia

			select @costoPromedio,@existenciaAnt,@ultimoValorCtoProm,@actualValorMov,@existencia
	...
	update doc_inv_movimiento_detalle
		set Disponible = ISNULL(@existencia,0),
			CostoPromedio = case when ISNULL(@costoPromedio,0) =0 then isnull(pe.CostoPromedio,0)
								  else ISNULL(@costoPromedio,0) 
							end,
			CostoUltimaCompra = case when ISNULL(@costoUltimaCompra,0) =0 then isnull(pe.CostoUltimaCompra,0)
		...
		update doc_inv_movimiento_detalle
		set 		
			ValCostoPromedio = isnull(CostoPromedio,0) * isnull(Disponible,0),
			ValCostoUltimaCompra = isnull(CostoUltimaCompra,0) * isnull(Disponible,0)
		from doc_inv_movimiento_detalle md		
		where MovimientoDetalleId = @pMovimientoDetalleId 
