...
namespace ERP.Common.PuntoVenta
{
    public partial class frmReimprimirTicket : Form
    {
        ...
        private void uiTicket_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            string folio = uiTicket.Rows[e.RowIndex].Cells["ID"].Value.ToString();
            ...
            if (esReimpresion)
            {
                if (folio != "")
                {
                    ...
                    entityReimpresion.CajaId = this.puntoVentaContext.cajaId;
                    entityReimpresion.CreadoEl = oContext.p_GetDateTimeServer().FirstOrDefault().Value;
                    entityReimpresion.CreadoPor = this.puntoVentaContext.usuarioId;
