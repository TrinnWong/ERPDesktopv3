...
namespace ERP.Common.Forms
{
    public partial class frmBuscarProducto : Form
    {
        ...
        public int sucursalFiltroId;
        public bool soloConExistencia;
            ...
            if (uiGridProducto.CurrentCell.RowIndex >= 0)
            {
               ...
               error = validarAgregarProducto();
                ...
                else
                {
                    ...
                    if (opcionERP == (int)opcionesERP.productosCompra)
                    {
                        //frmProductosCompra oForm = (frmProductosCompra)this.Owner.MdiChildren[0];

                        //oForm.AgregarProducto(productoSeleccionado);
                        //frmProductosCompra.GetInstance().buscarProductoF3(productoSeleccionado.clave);
                        
                        this.Close();
                    }
                    if (opcionERP == (int)opcionesERP.ajusteEntrada)
                    {
                       // frmAjusteUpd.GetInstance().buscarProductoF3(productoSeleccionado.clave);
                        this.Close();
                    }
                    if (opcionERP == (int)opcionesERP.entradaTraspaso)
                    {
                        //frmEntradaTraspasoUpd.GetInstance().buscarProductoF3(productoSeleccionado.clave);
                        this.Close();
                    }
                    if (opcionERP == (int)opcionesERP.entradaDirecta)
                    {
                        //frmEntradaDirectaUpd.GetInstance().buscarProductoF3(productoSeleccionado.clave);
                        this.Close();
                    }
                    if (opcionERP == (int)opcionesERP.kardexProducto)
                    {
                        //frmProductoKardex.GetInstance().buscarProductoF3(productoSeleccionado.clave);
                        this.Close();
                    }
                    if (opcionERP == (int)opcionesERP.apartadosUpd)
                    {
                        //frmProductoKardex.GetInstance().buscarProductoF3(productoSeleccionado.clave);
                        frmApartadosUpd.GetInstance().SeleccionadProducto( productoSeleccionado);
                        this.Close();
                    }
