...
namespace ERPv1.RH
{
    public partial class frmEstatusEmpleado : Form
    {
        Resultado resultado;
        EstatusEmpleadoInterface eeI;

        public frmEstatusEmpleado()
        {
            InitializeComponent();
            eeI = new EstatusEmpleadoInterface();
        }
        ...
        private void Buscar()
        {
            ...
            dgvEstatusDepto.DataSource = eeI.ConsultarListado(busqueda);
            dgvEstatusDepto.Refresh();
        }
        ...
        private Resultado ValidarAgregar(int clave, string nomEstatusDepto)
        {
            ...
            {
                DataTable dt = eeI.ConsultarRegistro(clave, false);
            ...
            if (nomEstatusDepto == "")
            {
                resultado.mensaje += "Capture el Estatus del Empleado.";
                resultado.ok = false;
            }
            return resultado;
        }

        private Resultado ValidarEditar(int clave, string nomEstatusDepto)
        {
            resultado = new Resultado();
            DataTable dt = eeI.ConsultarRegistro(clave, false);
            if (clave <= 0)
        ...
        private void Insertar()
        {
            ...
            string nomEstatusDepto = txtNombreDepartamento.Text.Trim();
            bool estatus = chkEstatus.Checked ? true : false;
            int? empresa = null;
            int? sucursal = null;
            if (!ValidarAgregar(clave, nomEstatusDepto).ok)
            {
                MessageBox.Show(resultado.mensaje, "Alerta", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            resultado = eeI.Agregar(clave, nomEstatusDepto, estatus, empresa, sucursal);
        ...
        private void Editar()
        {
            ...
            string nomEstatusDepto = txtNombreDepartamento.Text.Trim();
            bool estatus = chkEstatus.Checked ? true : false;
            if (!ValidarEditar(clave, nomEstatusDepto).ok)
            {
                MessageBox.Show(resultado.mensaje, "", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            resultado = eeI.Actualizar(clave, nomEstatusDepto, estatus);
            MessageBox.Show(resultado.mensaje, "", MessageBoxButtons.OK, MessageBoxIcon.Information);
            if (resultado.ok)
            {
                LimpiarControles();
        ...
        private void frmEstatusEmpleado_Load(object sender, EventArgs e)
        {
            Buscar();
        }
        ...
        private void dgvEstatusDepto_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            int rowIndex = e.RowIndex;
            if (rowIndex >= 0)
            {
                DataGridViewRow row = dgvEstatusDepto.Rows[rowIndex];
                try
                {
