...
{
    public partial class frmAjusteUpd : FormERP
    {
        int err;
        List<MovimientoInventarioProductoModel> lstMovs;


        private static frmAjusteUpd _instance;
        public int tipoMovimientoForm{get;set;}

        public static frmAjusteUpd GetInstance()
        {
            if (_instance == null) _instance = new frmAjusteUpd();
        ...
        private void frmSalidaTraspasoUpd_Load(object sender, EventArgs e)
        {
            ...
            switch (tipoMovimientoForm)
            {
                case 5:
                    ...
                    lblTitulo.Text = "Entrada Por Ajuste";
                    break;
                case 6:
                    lblTitulo.Text = "Salida Por Ajuste";
                    break;
                default:
                    break;
            }
        ...
        private void agregarProducto()
        {
            ...
            {
                MessageBox.Show("Es necesario especificar un producto y una cantidad", "AVISO");
            }
        }

        public void buscarProductoF3(string claveProd)
        {
            uiClave.Text = claveProd;
            buscarProducto();
        }
        #endregion
        ...
        private void guardar(bool autorizar)
        {
            try
            {
               ...
               // int sucursalDestino = uiSucursalDestino.SelectedValue == null ? 0 : int.Parse(uiSucursalDestino.SelectedValue.ToString());
                ...
                int? tipoMermaId = uiTipoMerma.EditValue == null ? null : (int?)Convert.ToInt32(uiTipoMerma.EditValue) ;
                switch (accionForm)
                {
                    /**********Agregar***************************/
                    case 1:
                        ...
                        {
                            oContext.p_doc_inv_movimiento_ins(pMovimientoID, sucursalOrigen, uiFolio.Text, tipoMovimientoForm, fechaActual, fechaActual.TimeOfDay,uiComentarios.Text.Trim(),
                          total, 1, false, null,sucursalOrigen, null, null, tipoMermaId);
                    ...
                    case 2:
                        using (TransactionScope scope = new TransactionScope())
                        {
                            ...
                            oContext.p_doc_inv_movimiento_upd(pMovimientoID, sucursalOrigen, uiFolio.Text, tipoMovimientoForm, fechaActual, fechaActual.TimeOfDay, uiComentarios.Text.Trim(),
                            total, 1, false, null,sucursalOrigen, null, null, tipoMermaId);
        ...
        public void buscarRegistro()
        {
            ...
            try
            {
                ...
                uiComentarios.Text = entity.Comentarios;
                uiTipoMerma.EditValue = entity.TipoMermaId;
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show("Ocurrió un error al intentar obtener el detalle del movimiento", "ERROR", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        ...
        private void boton_imprimir()
        {
            try
            {
                doc_inv_movimiento eMov = oContext.doc_inv_movimiento.Where(w => w.MovimientoId == idForm).FirstOrDefault();
        ...
        {
            rpt.rptMovimientoInventario oTicket = new rpt.rptMovimientoInventario();
        ...
        private void imprimirCancelacion()
        {
            ...
            ReportViewer oViewer = new ReportViewer();
            oContext = new ERPProdEntities();
            var ds = oContext.p_rpt_movimiento_cancela_inv(idForm,0).ToList();
            oTicket.DataSource = ds;
            oTicket.autorizadoPor = ds.FirstOrDefault().AutorizadoPor;
        ...
        private void frmAjusteUpd_KeyDown(object sender, KeyEventArgs e)
        {
           
        }

        private void frmAjusteUpd_KeyUp(object sender, KeyEventArgs e)
        {
        ...
        private void LoadTipoMerma()
        {
            try
            {
                uiTipoMerma.Properties.DataSource = oContext.cat_tipos_mermas
                    .ToList();
            }
            catch (Exception ex)
            {
