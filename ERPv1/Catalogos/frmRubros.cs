...
namespace ERPv1.Catalogos
{
    public partial class frmRubros : Form
    {
        ...
        private void Buscar()
        {
            ...
            dgvRubro.DataSource = fI.ConsultarListado(busqueda);
            dgvRubro.Refresh();
        }
        ...
        private void Insertar()
        {
            ...
            string nomFamilia = txtNomRubro.Text.Trim();
            ...
            resultado = fI.Agregar(clave, nomFamilia, estatus, empresa, sucursal);
        ...
        private void Editar()
        {
            ...
            resultado = fI.Actualizar(clave, nomFamilia, estatus);
            MessageBox.Show(resultado.mensaje, "", MessageBoxButtons.OK, MessageBoxIcon.Information);
            if (resultado.ok)
            {
                LimpiarControles();
        ...
        private int BuscarRegistro(int clave)
        {
            ...
            if (dt.Rows != null && dt.Rows.Count > 0)
            {
                ...
                nClave.Value = clave;
                txtNomRubro.Text = dt.Rows[0]["Descripcion"].ToString();
        ...
        {
            dgvRubro.DefaultCellStyle.ForeColor = Color.Black;
            Buscar();
        }

        private void txtNomRubro_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == Convert.ToChar(Keys.Enter))
            {
                chkEstatus.Focus();
        ...
        private void dgvRubro_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (rowIndex >= 0)
            {
                DataGridViewRow row = dgvRubro.Rows[rowIndex];
                try
                {
