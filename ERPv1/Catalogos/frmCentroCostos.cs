...
namespace ERPv1.Catalogos
{
    ...
    {
        Resultado resultado;
        CentroCostosInterface fI;

        public frmCentroCostos()
        {
            InitializeComponent();
            fI = new CentroCostosInterface();
        }
        ...
        private void Buscar()
        {
            ...
            dgvCentroCosto.DataSource = fI.ConsultarListado(busqueda);
            dgvCentroCosto.Refresh();
        }
        ...
        private void LimpiarControles()
        {
            nClave.Enabled = true;
            nClave.Value = 0;
            txtNomCentroCosto.Text = "";
            chkEstatus.Checked = true;
        }

        private Resultado ValidarAgregar(int clave, string nomCentroCosto)
        {
            ...
            if (nomCentroCosto == "")
            {
                resultado.mensaje += "Capture el nombre del Centro de Costos.";
                resultado.ok = false;
            }
            return resultado;
        }

        private Resultado ValidarEditar(int clave, string nomCentroCosto)
        {
            resultado = new Resultado();
            DataTable dt = fI.ConsultarRegistro(clave, false);
            if (clave <= 0)
            ...
            if (nomCentroCosto == "")
            {
                resultado.mensaje += "Capture el nombre del Centro de Costo.";
                resultado.ok = false;
            }
            return resultado;
        }
        ...
        private void Insertar()
        {
            ...
            string nomCentroCosto = txtNomCentroCosto.Text.Trim();
            bool estatus = chkEstatus.Checked ? true : false;
            int? empresa = null;
            int? sucursal = null;
            if (!ValidarAgregar(clave, nomCentroCosto).ok)
            {
                MessageBox.Show(resultado.mensaje, "Alerta", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            resultado = fI.Agregar(clave, nomCentroCosto, estatus, empresa, sucursal);
        ...
        private void Editar()
        {
            ...
            string nomCentroCosto = txtNomCentroCosto.Text.Trim();
            bool estatus = chkEstatus.Checked ? true : false;
            if (!ValidarEditar(clave, nomCentroCosto).ok)
            {
                MessageBox.Show(resultado.mensaje, "", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            resultado = fI.Actualizar(clave, nomCentroCosto, estatus);
            MessageBox.Show(resultado.mensaje, "", MessageBoxButtons.OK, MessageBoxIcon.Information);
            if (resultado.ok)
            {
                LimpiarControles();
        ...
        private int BuscarRegistro(int clave)
        {
            ...
            if (dt.Rows != null && dt.Rows.Count > 0)
            {
                ...
                nClave.Value = clave;
                txtNomCentroCosto.Text = dt.Rows[0]["Descripcion"].ToString();
        ...
        {
            dgvCentroCosto.DefaultCellStyle.ForeColor = Color.Black;
            Buscar();
        }
        ...
        private void nClave_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == Convert.ToChar(Keys.Enter))
            {
                ...
                if (numero == 0)
                {
                    txtNomCentroCosto.Focus();
                }
            }
        }

        private void txtNomCentroCosto_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == Convert.ToChar(Keys.Enter))
            {
                chkEstatus.Focus();
        ...
        private void dgvCentroCosto_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            int rowIndex = e.RowIndex;
            if (rowIndex >= 0)
            {
                DataGridViewRow row = dgvCentroCosto.Rows[rowIndex];
                try
                {
