...
{
    public partial class frmAndenes : Form
    {
        Resultado resultado;
        AndenesInterface aI;
        ...
        private void Buscar()
        {
            ...
            dgvAndan.DataSource = aI.ConsultarListado(busqueda);
            dgvAndan.Refresh();
        }
        ...
        private void LimpiarControles()
        {
            nClave.Enabled = true;
            nClave.Value = 0;
            txtNombreAndanes.Text = "";
            chkEstatus.Checked = true;
        }

        private Resultado ValidarAgregar(int clave, string nomAndenes)
        {
            ...
            if (nomAndenes == "")
            {
                resultado.mensaje += "Capture el nombre del Anden.";
                resultado.ok = false;
            }
            return resultado;
        }

        private Resultado ValidarEditar(int clave, string nomAndenes)
        {
            resultado = new Resultado();
            DataTable dt = aI.ConsultarRegistro(clave, false);
            if (clave <= 0)
        ...
        private void Insertar()
        {
            ...
            string nomAndenes = txtNombreAndanes.Text.Trim();
            bool estatus = chkEstatus.Checked ? true : false;
            int? empresa = null;
            int? sucursal = null;
            if (!ValidarAgregar(clave, nomAndenes).ok)
            {
                MessageBox.Show(resultado.mensaje, "Alerta", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            resultado = aI.Agregar(clave, nomAndenes, estatus, empresa, sucursal);
        ...
        private void Editar()
        {
            ...
            string nomAndenes = txtNombreAndanes.Text.Trim();
            bool estatus = chkEstatus.Checked ? true : false;
            if (!ValidarEditar(clave, nomAndenes).ok)
            {
                MessageBox.Show(resultado.mensaje, "", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            resultado = aI.Actualizar(clave, nomAndenes, estatus);
            MessageBox.Show(resultado.mensaje, "", MessageBoxButtons.OK, MessageBoxIcon.Information);
            if (resultado.ok)
            {
                LimpiarControles();
        ...
        private int BuscarRegistro(int clave)
        {
            ...
            if (dt.Rows != null && dt.Rows.Count > 0)
            {
                ...
                nClave.Value = clave;
                txtNombreAndanes.Text = dt.Rows[0]["Andenes"].ToString();
        ...
        private void dgvAndan_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            int rowIndex = e.RowIndex;
            if (rowIndex >= 0)
            {
                DataGridViewRow row = dgvAndan.Rows[rowIndex];
                try
                {
        ...
        private void frmAndenes_Load(object sender, EventArgs e)
        {
            dgvAndan.DefaultCellStyle.ForeColor = Color.Black;
            Buscar();
        }
