...
{
    public partial class frmGruposSat : Form
    {
        Resultado resultado;
        GruposSatInterface gI;

        public frmGruposSat()
        {
            InitializeComponent();
            gI = new GruposSatInterface();
        }
        ...
        private void Buscar()
        {
            ...
            dgvGruposSat.DataSource = gI.ConsultarListado(busqueda);
            dgvGruposSat.Refresh();
        }
        ...
        private Resultado ValidarAgregar(int clave, string nomGrupo,string codigoSAT)
        {
            ...
            if (nomGrupo == "")
            {
                resultado.mensaje += "Capture el nombre del Grupo.";
                resultado.ok = false;
            }
            if (codigoSAT == "")
            {
        ...
        private Resultado ValidarEditar(int clave, string nomGrupo,string codigoSAT)
        {
            resultado = new Resultado();
            DataTable dt = gI.ConsultarRegistro(clave, false);
            if (clave <= 0)
        ...
        private void Insertar()
        {
            ...
            string nomGrupo = txtNombreGrupo.Text.Trim();
            int? empresa = null;
            int? sucursal = null;
            if (!ValidarAgregar(clave, nomGrupo,uiCodigoSAT.Text).ok)
            {
                MessageBox.Show(resultado.mensaje, "Alerta", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            resultado = gI.Agregar(clave, nomGrupo, empresa, sucursal,uiCodigoSAT.Text);
        ...
        private void Editar()
        {
            ...
            int? empresa = null;
            int? sucursal = null;
            if (!ValidarEditar(clave, nomGrupo,uiCodigoSAT.Text).ok)
            {
                MessageBox.Show(resultado.mensaje, "", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            resultado = gI.Actualizar(clave, nomGrupo,uiCodigoSAT.Text);
            MessageBox.Show(resultado.mensaje, "", MessageBoxButtons.OK, MessageBoxIcon.Information);
            if (resultado.ok)
            {
                LimpiarControles();
        ...
        private int BuscarRegistro(int clave)
        {
            ...
            if (dt.Rows != null && dt.Rows.Count > 0)
            {
                ...
                nClave.Value = clave;
                txtNombreGrupo.Text = dt.Rows[0]["Descripcion"].ToString(); 
        ...
        private void dgvGruposSat_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            int rowIndex = e.RowIndex;
            if (rowIndex >= 0)
            {
                DataGridViewRow row = dgvGruposSat.Rows[rowIndex];
                try
                {
        ...
        private void frmGruposSat_Load(object sender, EventArgs e)
        {
            dgvGruposSat.DefaultCellStyle.ForeColor = Color.Black;
            Buscar();
        }
