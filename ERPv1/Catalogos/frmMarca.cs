...
namespace ERPv1.Catalogos
{
    public partial class frmMarca : Form
    {
        ...
        private void Buscar()
        {
            ...
            dgvMarca.DataSource = eI.ConsultarListado(busqueda);
            dgvMarca.Refresh();
        }
        ...
        private Resultado ValidarAgregar(int clave, string nomMaarca)
        {
            ...
            if (nomMaarca == "")
            {
                resultado.mensaje += "Capture el nombre de la Marca.";
                resultado.ok = false;
            }
            return resultado;
        }

        private Resultado ValidarEditar(int clave, string nomMaarca)
        {
            resultado = new Resultado();
            DataTable dt = eI.ConsultarRegistro(clave, false);
            if (clave <= 0)
        ...
        private void Insertar()
        {
            ...
            string nomMarca = txtNombreMarca.Text.Trim();
            bool estatus = chkEstatus.Checked ? true : false;
            int? empresa = null;
            int? sucursal = null;
            if (!ValidarAgregar(clave, nomMarca).ok)
            {
                MessageBox.Show(resultado.mensaje, "Alerta", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            resultado = eI.Agregar(clave, nomMarca, estatus, empresa, sucursal);
        ...
        private void Editar()
        {
            ...
            string nomMarca = txtNombreMarca.Text.Trim();
            bool estatus = chkEstatus.Checked ? true : false;
            if (!ValidarEditar(clave, nomMarca).ok)
            {
                MessageBox.Show(resultado.mensaje, "", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            resultado = eI.Actualizar(clave, nomMarca, estatus);
            MessageBox.Show(resultado.mensaje, "", MessageBoxButtons.OK, MessageBoxIcon.Information);
            if (resultado.ok)
            {
                LimpiarControles();
        ...
        private int BuscarRegistro(int clave)
        {
            ...
            if (dt.Rows != null && dt.Rows.Count > 0)
            {
                ...
                nClave.Value = clave;
                txtNombreMarca.Text = dt.Rows[0]["Marcas"].ToString();
        ...
        {
            dgvMarca.DefaultCellStyle.ForeColor = Color.Black;
            Buscar();
        }
        ...
        private void dgvMarca_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            if (rowIndex >= 0)
            {
                DataGridViewRow row = dgvMarca.Rows[rowIndex];
                try
                {
