...
{
    public partial class frmAnaqueles : Form
    {
        Resultado resultado;
        AnaquelesInterface aI;

        public frmAnaqueles()
        {
            InitializeComponent();
            aI = new AnaquelesInterface();
        }
        ...
        private void Buscar()
        {
            ...
            dgvAnaquel.DataSource = aI.ConsultarListado(busqueda);
            dgvAnaquel.Refresh();
        }
        ...
        private void LimpiarControles()
        {
            nClave.Enabled = true;
            nClave.Value = 0;
            txtNombreAnaquel.Text = "";
            chkEstatus.Checked = true;
        }

        private Resultado ValidarAgregar(int clave, string nomAnaquel)
        {
            ...
            if (nomAnaquel == "")
            {
                resultado.mensaje += "Capture el nombre del Anaquel.";
                resultado.ok = false;
            }
            return resultado;
        }

        private Resultado ValidarEditar(int clave, string nomAnaquel)
        {
            resultado = new Resultado();
            DataTable dt = aI.ConsultarRegistro(clave, false);
            if (clave <= 0)
        ...
        private void Insertar()
        {
            ...
            string nomAnaquel = txtNombreAnaquel.Text.Trim();
            bool estatus = chkEstatus.Checked ? true : false;
            int? empresa = null;
            int? sucursal = null;
            if (!ValidarAgregar(clave, nomAnaquel).ok)
            {
                MessageBox.Show(resultado.mensaje, "Alerta", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            resultado = aI.Agregar(clave, nomAnaquel, estatus, empresa, sucursal);
        ...
        private void Editar()
        {
            ...
            string nomAnaquel = txtNombreAnaquel.Text.Trim();
            bool estatus = chkEstatus.Checked ? true : false;
            if (!ValidarEditar(clave, nomAnaquel).ok)
            {
                MessageBox.Show(resultado.mensaje, "", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            resultado = aI.Actualizar(clave, nomAnaquel, estatus);
            MessageBox.Show(resultado.mensaje, "", MessageBoxButtons.OK, MessageBoxIcon.Information);
            if (resultado.ok)
            {
                LimpiarControles();
        ...
        private int BuscarRegistro(int clave)
        {
            ...
            if (dt.Rows != null && dt.Rows.Count > 0)
            {
                ...
                nClave.Value = clave;
                txtNombreAnaquel.Text = dt.Rows[0]["Nombre Anaquel"].ToString(); ;
        ...
        private void nClave_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == Convert.ToChar(Keys.Enter))
            {
                ...
                if (numero == 0)
                {
                    txtNombreAnaquel.Focus();
                }
            }
        }

        private void txtNombreAnaquel_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == Convert.ToChar(Keys.Enter))
            {
                chkEstatus.Focus();
        ...
        private void dgvAnaquel_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            int rowIndex = e.RowIndex;
            if (rowIndex >= 0)
            {
                DataGridViewRow row = dgvAnaquel.Rows[rowIndex];
                try
                {
        ...
        private void frmAnaqueles_Load(object sender, EventArgs e)
        {
            dgvAnaquel.DefaultCellStyle.ForeColor = Color.Black;
            Buscar();
        }
